...
describe("Testes model jobs", () => {
    ...
    test("Testar a criação de um novo job com todas informações", () => {
        ...
        const sut = new Job("description", "company", today, recruiterUser, true, 10, "id", [candidateUser]);
    ...
    test("Testar o erro quando o Usuario não é Candidate", () => {
        const today = new Date();
        const recruiterUser = new User("joao", "joaosilva", "RECRUITER", "Growdev");
        expect(() => new Job("description", "company", today, recruiterUser, true, 10, "id", [recruiterUser])).toThrowError();
    })
    ...
    test("testar metodo toJson", () => {
        ...
        const recruiterUser = new User("joao", "joaosilva", "RECRUITER", "Growdev");
        const candidateUser = new User("joao", "joaosilva", "CANDIDATE", "Growdev");
        const sut = new Job("description", "comgrowdevpany", today, recruiterUser, true, 10, "id", [candidateUser]);
    ...
    test("testar metodo isFull", () => {
        ...
        const sutSegundo = new Job("description", "comgrowdevpany", today, recruiterUser, true, 1, "id", [candidateUser]);
        expect(sutSegundo.isFull()).toBeTruthy();
        const sutTerceiro = new Job("description", "comgrowdevpany", today, recruiterUser, true);
